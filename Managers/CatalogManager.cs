    ...
    ILogger<CatalogManager> logger) : ICatalogManager, IManager
{
    ...
    {
        var generatedAgents = LoadGeneratedAgents(language);
        if (generatedAgents.Count > 0)
        {
            return generatedAgents;
        }

        logger.LogInformation("Generated agent catalog is empty for {language}. Falling back to runtime econ item discovery.", language);
    ...
    private IReadOnlyList<AgentOption> LoadGeneratedAgents(string language)
    {
        var filePath = ResolveLocalizedGeneratedFile("agents", language);
        var data = ReadJson<List<AgentData>>(filePath);
        ...
        agents.AddRange(data
            .Select(entry => ResolveTeam(entry.Team) is { } team
                ? new AgentOption((ushort)entry.ItemId, team, entry.AgentName)
                : null)
    ...
    private static CStrikeTeam? ResolveTeam(int team)
        => team switch
        {
            2 => CStrikeTeam.TE,
            3 => CStrikeTeam.CT,
