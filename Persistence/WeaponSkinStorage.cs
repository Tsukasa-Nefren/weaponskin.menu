...
namespace WeaponSkin.Menu.Persistence;
    ...
    ILogger<WeaponSkinStorage> logger) : IWeaponSkinStorage, IManager
{
    ...
    public Task<TeamItem[]> GetPlayerTeamGloves(SteamID steamId)
        => GetPlayerTeamItems<TeamGloveEntity>(steamId);
    ...
    public Task SaveGloves(SteamID steamId, CStrikeTeam team, EconItemId itemId)
        => SaveTeamItem(new TeamGloveEntity
        {
            SteamId = ToSteamIdValue(steamId),
            Team = (int)team,
            ItemId = (int)itemId,
    ...
    public Task ClearGloves(SteamID steamId, CStrikeTeam team)
        => ClearTeamItem<TeamGloveEntity>(steamId, team);
    ...
    private string BuildSqliteConnectionString(string? databaseName)
    {
        ...
        logger.LogInformation("WeaponSkin.Menu is using SQLite database: {path}", dbPath);
